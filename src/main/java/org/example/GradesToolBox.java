...
public class GradesToolBox {
     ...
     */
    public double pctToGpaIf(String pctRange){
        if(!pctRange.isEmpty()){
            int percentIndex = pctRange.indexOf('%'); // locates position of first % in string
            double pct = -1.0;
            try {
                if (percentIndex != -1) {
                    pct = Double.parseDouble(pctRange.substring(0, percentIndex).trim()); // parses first substring before %
                } else {
     ...
     *
     * @param gpaScore grade represented from 0 to 4.0.
     * @exception NumberFormatException if input string is not well-formed or empty
